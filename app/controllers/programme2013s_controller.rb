class Programme2013sController < InheritedResources::Base
  ...
  def index
    @title =  Time.now.year
    #The date is included into the ? by ActiveRecord, which also extracts the day for us. Thus, only the present (today's) and future events are shown up, the rest are hidden.
  ...
  def create
    ...
    respond_to do |format|
      if @programme2013.save
        format.html { redirect_to @programme2013, notice: 'Programme was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @programme2013.update(programme2013_params)
        format.html { redirect_to @programme2013, notice: 'Programme was successfully updated.' }
